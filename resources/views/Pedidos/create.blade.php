...
<div class="py-8">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <x-ui.card>
            <h1 class="text-2xl font-bold text-gray-900 mb-6">Nuevo Pedido</h1>
            ...
            <form action="{{ route('pedidos.store') }}" method="POST">
                ...
                <div class="mb-5">
                    ...
                    @foreach($categorias as $categoria)
                        <details class="mb-3 border border-orange-200 rounded-xl overflow-hidden">
                            ...
                            <div class="p-4 space-y-2">
                                @forelse($categoria->productos as $producto)
                                    <div class="flex justify-between items-center bg-gray-50 rounded-lg px-4 py-2.5">
                                        <div class="{{ $producto->estado === 'agotado' ? 'line-through text-gray-400' : 'text-sm text-gray-800' }}">
                                            {{ $producto->nombre }}
                                        ...
                                        @if($producto->estado !== 'agotado')
                                            <input type="number"
                                                   ...
                                                   class="w-20 border border-orange-200 rounded-lg px-3 py-1.5 text-sm text-center focus:ring-2 focus:ring-orange-400 focus:border-orange-400 producto-cantidad">
                                        @endif
                                    </div>
                                @empty
                                    <p class="text-sm text-gray-500">Sin productos en esta categoría</p>
                                @endforelse
                            </div>
                        </details>
                    @endforeach
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <x-ui.form-input name="fecha" label="Fecha de Entrega" type="date" :value="date('Y-m-d')" :required="true"/>
                ...
                <div class="flex items-center gap-3 pt-4">
                    <x-ui.button type="submit">Guardar Pedido</x-ui.button>
