  ...
  <div class="container mx-auto px-4 sm:px-6 lg:px-8">
    <div class="bg-white rounded-2xl shadow-lg p-8">
      ...
      <h1 class="text-3xl font-semibold mb-6 text-gray-800 text-center">
        Editar Valoración {{ $valoracion->id }}
      </h1>
      ...
      <form action="{{ route('valoraciones.update', $valoracion) }}" method="POST">
        ...
        <div class="mb-6">
          ...
          <select name="cliente_id" required
            ...
            @foreach($clientes as $cliente)
              <option value="{{ $cliente->id }}" {{ old('cliente_id', $valoracion->cliente_id) == $cliente->id ? 'selected' : '' }}>
        ...
        <div class="mb-6">
          ...
          <select name="valoracion" required
                  class="w-full border border-orange-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-orange-400">
            @for($i = 1; $i <= 5; $i++)
              <option value="{{ $i }}" {{ old('valoracion', $valoracion->valoracion) == $i ? 'selected' : '' }}>
                {{ $i }} estrella{{ $i > 1 ? 's' : '' }}
              </option>
            @endfor
          </select>
        ...
        <div class="mb-6">
          ...
          <textarea name="comentario" rows="4"
                    class="w-full border border-orange-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-orange-400"
                    placeholder="Opcional">{{ old('comentario', $valoracion->comentario) }}</textarea>
        </div>
